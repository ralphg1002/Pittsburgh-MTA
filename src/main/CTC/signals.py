...
class CTCTrackController(QObject):
    sendAuthority = pyqtSignal(int, int, int, int) # line (1 for green, 2 for red), block number, authority
    sendSuggestedSpeed = pyqtSignal(int, int, int, int) # line, block number, suggested speed
    sendMaintenance = pyqtSignal(int, int, int, bool) # line, block number, 1 if disabled, 0 if enabled

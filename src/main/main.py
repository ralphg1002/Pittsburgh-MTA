...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.pixmapMTALogo = self.pixmapMTALogo.scaled(math.floor(1862 * 0.25), math.floor(2046 * 0.25))
        self.logo = QLabel(self)
        self.logo.setPixmap(self.pixmapMTALogo)
        self.logo.adjustSize()
        self.logo.move(20, math.floor(self.bodyBlock.height()/2 - self.logo.height() / 2))
        self.logo.show()
    ...
    def box_button(self, button, width, height):
        ...
        button.setStyleSheet('color: ' + self.colorDarkBlue +
    ...
    def set_relative_right(self, child, parent, pad):
        child.move(parent.x() + parent.width() + pad, parent.y() + math.floor((parent.height() - child.height()) / 2.0))
